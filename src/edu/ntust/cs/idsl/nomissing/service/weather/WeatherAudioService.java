...
public class WeatherAudioService extends WeatherService {
    ...
    private static final String ACTION = "edu.ntust.cs.idsl.nomissing.action.GET_WEATHER_AUDIO";
    ...
    protected void handleAction(Bundle extras) {
        ...
        for (final Weather weather : weatherList) {
            ...
            NoMissingHttpClient.download(audioURL,
                    new BinaryHttpResponseHandler(allowedContentTypes) {
                        ...
                        public void onSuccess(int statusCode, Header[] headers, byte[] response) {
                            ...
                            sendBroadcast(WeatherServiceReceiver.getActionReceiveWeatherServiceUpdateProgress(WeatherAudioService.this, prograss));
                        }

                        @Override
                        public void onFailure(int statusCode, Header[] headers, byte[] response, Throwable throwable) {
                            Log.i(TAG, throwable.toString());
                            sendBroadcast(WeatherServiceReceiver.getActionReceiveWeatherServiceFailure(WeatherAudioService.this));
                        }
                    });
        }

        sendBroadcast(WeatherServiceReceiver.getActionReceiveWeatherServiceFinish(WeatherAudioService.this));;
